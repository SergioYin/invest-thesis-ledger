# Ledger Schema v1.7.2

This document defines the JSON ledger format accepted by `invest-thesis-ledger`
v1.7.2. Ledgers are research organization records only and are not investment
advice.
...
| --- | --- | --- |
| `ledger_version` | string | Schema version. v1.7.2 ledgers should use `"1.7.2"`. v0.1.0 through v1.7.1 remain accepted for compatibility; other values validate with a warning. |
...
For the same input file or ordered input file list, v1.7.2 CLI outputs are
deterministic:
...
writes a deterministic starter ledger with v1.7.2 fields, fixed placeholder
