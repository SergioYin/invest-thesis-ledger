# Ledger Schema v1.7.3

This document defines the JSON ledger format accepted by `invest-thesis-ledger`
v1.7.3. Ledgers are research organization records only and are not investment
advice.
...
| --- | --- | --- |
| `ledger_version` | string | Schema version. v1.7.3 ledgers should use `"1.7.3"`. v0.1.0 through v1.7.2 remain accepted for compatibility; other values validate with a warning. |
...
For the same input file or ordered input file list, v1.7.3 CLI outputs are
deterministic:
...
- Core Markdown reports normalize ledger-controlled inline text so pipes,
  brackets, and line breaks do not corrupt headings, lists, tables, or source
  labels.
...
writes a deterministic starter ledger with v1.7.3 fields, fixed placeholder
...
absolute or external generated paths, rejects duplicate generated file entries,
rejects symlink generated or hash-listed files before hashing bytes,
recomputes SHA-256 hashes for every `file_hashes` entry, confirms matching
manifest/summary ledger IDs and tool versions, confirms archive counts,
confirms that `archive-summary.json` is excluded from `file_hashes` while
remaining in `generated_files`, and rejects workflow or dependency files. It
prints a deterministic validation summary and exits 0 for valid archives, 1
for content validation failures, and 2 for unreadable or malformed archive
inputs.
...
validation errors. When both archives are valid, the command compares verified
generated file lists, archive-summary `file_hashes`, `ledger_ids`,
`tool_version`, and file counts. It writes deterministic Markdown and JSON
with added, removed, changed, and unchanged file information plus old/new
ledger IDs, old/new tool versions, old/new file counts, and a `changed` or
`unchanged` status.
