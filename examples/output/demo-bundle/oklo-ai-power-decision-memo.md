# Decision Memo: Oklo AI Power Demand Thesis

> This is a research organization tool, not investment advice.

## Asset / Thesis Snapshot

- Ledger ID: oklo-ai-power
- Updated: 2026-05-12
- Asset: OKLO (Oklo Inc.)
- Type: equity
- Thesis: Advanced nuclear developers may benefit if AI data center demand increases the value of firm, clean power, but commercialization timing and regulatory execution remain the core constraints.

### Assumptions

- A1: AI data center operators continue seeking long-duration clean power contracts. (confidence: medium; sources: [S2])
- A2: Oklo can preserve financing flexibility through the next licensing milestones. (confidence: low; sources: [S1], [S3])

## Latest Review

- Date: 2026-06-30
- Decision: watch
- Summary: Maintain watchlist status until catalysts move from narrative support to dated, source-backed milestones.
- Drift: Catalyst tracking added so future reviews separate power-demand validation from Oklo-specific execution evidence.
- Sources: [S1], [S3]

## Broker View Summary

- Views: 2
- Rating cautious: 1
- Rating constructive: 1

| Institution | Rating | Target | As Of | Thesis | Sources |
| --- | --- | --- | --- | --- | --- |
| Grid Infrastructure Desk | constructive | watch for contracted megawatts | 2026-05-10 | Firm clean power scarcity could support premium economics if Oklo converts interest into binding contracts. | [S1], [S2] |
| Regulatory Risk Desk | cautious | no target until licensing timeline is dated | 2026-05-11 | Commercial upside remains hard to underwrite before licensing cadence and financing runway are clearer. | [S3] |

## High Risks

- R1: Licensing delay (severity: high; probability: medium; tags: regulatory, timeline; mitigation: Require dated regulatory milestones before increasing conviction.; sources: [S3])

## Catalyst Checklist

- [ ] CAT2: Material licensing update with a clear review timeline. (date: 2026-08-15) (status: watch; sources: [S3])
- [ ] CAT1: Signed commercial power purchase agreement with credible counterparty. (window: 2026-H2) (status: watch; sources: [S1], [S2])
- [ ] CAT3: Capital raise or partnership that reduces funding risk. (window: next financing update) (status: watch; sources: [S1])

## Exposure / Open Position Rules

- Position note: Watchlist only until milestone evidence improves.
- Position note: No thesis action without updated cash runway and licensing review.
- [ ] P1: Keep status at watchlist until a dated licensing update is source-backed. (exposure: watchlist; tags: regulatory, timeline; sources: [S3])
- [ ] P2: Do not size from AI power narrative without named contract evidence. (exposure: no position; tags: valuation, narrative; sources: [S1], [S2])
- [ ] C1: Confirm latest cash balance and expected burn. (open)
- [ ] C2: Map each announced customer to binding contract evidence. (open)

## Evidence / Stale-Source Summary

- Tracked Items: 13
- Supported Items: 13
- Unsupported Items: 0
- Sources: 3
- Unused Sources: 0
- Stale Sources: 0
- No stale sources detected.

## Questions before action

- [ ] Q1: Does the latest review decision still support the contemplated action?
- [ ] Q2: Are all high-severity risks understood with current mitigation evidence?
- [ ] Q3: Have open catalysts been checked for new source-backed status changes?
- [ ] Q4: Do open position rules permit the proposed exposure?
- [ ] Q5: Are stale, unused, or unsupported evidence gaps acceptable before action?
- [ ] Q6: What evidence would falsify the action after execution?

## Sources

- [S1] Company investor presentation. Oklo, 2026-05-01. https://example.com/oklo-investor-presentation
- [S2] Electricity demand outlook. Grid Research Desk, 2026-04-20. https://example.com/electricity-demand-outlook
- [S3] Nuclear licensing status note. Regulatory Monitor, 2026-04-28. https://example.com/nuclear-licensing-status
