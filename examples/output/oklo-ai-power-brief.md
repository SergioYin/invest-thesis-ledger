# Oklo AI Power Demand Thesis

> This is a research organization tool, not investment advice.

## Asset

- Name: Oklo Inc.
- Type: equity
- Ticker: OKLO
- Ledger ID: oklo-ai-power
- Updated: 2026-05-12

## Thesis

Advanced nuclear developers may benefit if AI data center demand increases the value of firm, clean power, but commercialization timing and regulatory execution remain the core constraints.

## Assumptions

- A1: AI data center operators continue seeking long-duration clean power contracts. (confidence: medium; sources: [S2])
- A2: Oklo can preserve financing flexibility through the next licensing milestones. (confidence: low; sources: [S1], [S3])

## Catalysts

- Signed commercial power purchase agreement with credible counterparty.
- Material licensing update with a clear review timeline.
- Capital raise or partnership that reduces funding risk.

## Current Position Notes

- Watchlist only until milestone evidence improves.
- No thesis action without updated cash runway and licensing review.

## Sources

- [S1] Company investor presentation. Oklo, 2026-05-01. https://example.com/oklo-investor-presentation
- [S2] Electricity demand outlook. Grid Research Desk, 2026-04-20. https://example.com/electricity-demand-outlook
- [S3] Nuclear licensing status note. Regulatory Monitor, 2026-04-28. https://example.com/nuclear-licensing-status
