# Portfolio Evidence Audit

> This is a research organization tool, not investment advice.

- Ledgers: 2
- Tracked Items: 28
- Supported Items: 22
- Unsupported Items: 6
- Sources: 5
- Unused Sources: 0
- Stale Sources: 0
- Duplicate Source URLs: 0

## Ledger Scores

| Rank | Score | Ticker | Ledger ID | Updated | Tracked | Unsupported | Unused Sources | Stale Sources |
| ---: | ---: | --- | --- | --- | ---: | ---: | ---: | ---: |
| 1 | 92 | OKLO | oklo-ai-power | 2026-06-30 | 15 | 2 | 0 | 0 |
| 2 | 81 | LEV2X | leveraged-etf-discipline | 2026-05-19 | 13 | 4 | 0 | 0 |

## Field Coverage

| Field | Tracked | Supported | Unsupported |
| --- | ---: | ---: | ---: |
| assumption | 4 | 4 | 0 |
| broker_view | 2 | 2 | 0 |
| catalyst | 5 | 4 | 1 |
| checklist | 5 | 0 | 5 |
| position_rule | 4 | 4 | 0 |
| review | 4 | 4 | 0 |
| risk | 4 | 4 | 0 |

## Unsupported Items

| Ledger ID | Ticker | Type | ID |
| --- | --- | --- | --- |
| leveraged-etf-discipline | LEV2X | catalyst | CAT1 |
| leveraged-etf-discipline | LEV2X | checklist | C1 |
| leveraged-etf-discipline | LEV2X | checklist | C2 |
| leveraged-etf-discipline | LEV2X | checklist | C3 |
| oklo-ai-power | OKLO | checklist | C1 |
| oklo-ai-power | OKLO | checklist | C2 |

## Unused Sources

- No unused sources detected.

## Stale Sources

- No stale sources detected.

## Duplicate Source URLs

- No duplicate source URLs across ledgers detected.
